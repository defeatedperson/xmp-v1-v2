  ...
  <div class="node-logs-page">
    ...
    <div class="page-header">
      ...
      <div class="header-right">
        ...
        <el-button :icon="Refresh" circle @click="fetchNodeLogs" :disabled="!selectedNodeId" />
      </div>
    </div>
...
const selectedDayLabel = computed(() => {
  const hit = dayOptions.find(x => x.value === selectedDayOffset.value)
  return hit ? hit.label : '今天'
})
...
const fetchNodeLogs = async () => {
  ...
  try {
    const response = await fetch(
      `/api/forward/${selectedNodeId.value}/log/list?d=${selectedDayOffset.value}`
    )
    if (!response.ok) {
...
const viewLog = (log) => {
  ...
  modalContent.value = `日期: ${selectedDayLabel.value}
ID: ${log.id}
